...
[How one scan runs](../chapter-3/how_one_scan_runs.md) is the same pipeline with the exact control-flow rules (`Skip` / `Exit` / `ReprocessAsType`, requested extraction, `filter_root`, observer, stop condition). This chapter stays at the model.
