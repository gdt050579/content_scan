...
`ScannerBuilder<T>` (optionally `ScannerBuilder<T, M>`) is how a [`Scanner`](scanner.md) comes into existence. Register plugins, optionally attach a filter, an [observer](observer.md), a [stop condition](stop_condition.md), a depth limit, and a type-change cap, then `build()`.
...
| `max_change_type(n)`                | Cap on [`ReprocessAsType`](../chapter-2/analyzer.md#analysisoutcome) per object. Clamped to `1..=T::COUNT`. Default `2`. |
...
An empty builder is legal: no identifiers, no analyzers, no extractors, no filter, no observer, no stop condition, `store_findings` true, `max_depth` 8, `max_change_type` 2. `scan()` then records objects (and runs nothing on them) unless the root is filtered out. Useful for tests; production scanners register at least the plugins they need.
