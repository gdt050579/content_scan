...
It cannot `Skip`, `Exit`, or `ReprocessAsType`. Steering stays with analyzers (`AnalysisOutcome`) and with [`StopCondition`](stop_condition.md). It cannot rewrite the filter. It does not see bytes, only paths and the finding strings analyzers already produced. `on_scan_object` fires again after an accepted `ReprocessAsType`, with the new type.
